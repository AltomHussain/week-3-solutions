...
/**
 * // Example 2)
 * // Additional solution to function `journeyPlanner` - as function
 * 
 * function journeyPlanner(locations, transportMode) {
  // Implement the function body
  const targetLocation = locations
    ...
    }) // filter correct location details
  
  const namedLocations = targetLocation
...
/**
 * // Example 3) Advanced
 * // Refactored solution, you rarely need to pass a function that call another function in array method
 * 
 * function journeyPlanner(locations, transportMode) {
 
  var travelablelocations = locations
    .filter(p=>isAccessibleByTransportMode(p,transportMode))
    .map(getLocationName)
